...
from inventario.forms import (BodegaStackForm, BodegaForm, ActaEntradaForm, 
...
class ArticuloResource(resources.ModelResource):
	...
	class Meta:
		...
		fields = ('nombre','fabricante__nombre','cantidad_minima_en_bodega','cantidad_minima_de_compra','fecha_creacion','tiempo_entrega')
		export_oder = ('nombre','fabricante__nombre','cantidad_minima_en_bodega','cantidad_minima_de_compra','fecha_creacion','tiempo_entrega')
	#end class
#end class

class ArticuloAdmin(ExportMixin, admin.ModelAdmin):
	list_display = ('nombre','thumbnail','fabricante','cantidad_minima_en_bodega','cantidad_minima_de_compra','fecha_creacion','tiempo_entrega','precio_')
...
class BodegaAdmin(ExportMixin, admin.ModelAdmin):
	list_display = ('nombre','identi','direccion','telefono','precio_activos','precio_activos_no_serial','capital_en_bodega')
	search_fields = ('nombre','identi','direccion','telefono')
	form = BodegaForm
	inlines = (CuentaStaked,)
	fieldsets = [(None,{'classes':('suit-tab','suit-tab-bodega'),'fields':['identi','nombre','direccion','telefono','valor_activo','valor_activo_no_serial','valor_total','activos','noserial']})]
...
class ArticuloNoResource(resources.ModelResource):
	...
	class Meta:
		...
		fields =  ('nombre','fabricante__nombre','unidad','cantidad_minima_en_bodega','cantidad_minima_de_compra','fecha_creacion','precio','tiempo_entrega')
		export_order = 	('nombre','fabricante__nombre','unidad','cantidad_minima_en_bodega','cantidad_minima_de_compra','fecha_creacion','precio','tiempo_entrega','activado')
	#end class
#end class


class ArticuloNoSerialAdmin(ExportMixin, admin.ModelAdmin):
	list_display = ('nombre','thumbnail','fabricante','unidad','cantidad_minima_en_bodega','cantidad_minima_de_compra','fecha_creacion','precio','tiempo_entrega','activado')
	search_fields = ('nombre','descripcion','fabricante')
...
class ActivoResource(resources.ModelResource):
	...
	class Meta:
		...
		export_order =  ('articulo__nombre','tipo__nombre','bodega__nombre','serial','precio','activado','instalado')
	#end class
#end class
...
class ActaRequisionAdmin(admin.ModelAdmin):
	...
	suit_form_tabs = (('actarequisicion','1.Acta de requesición'),('requesicionarticulo','2.Activos'),('requesicionnoserial','3.Activos no serial'))
#end class
...
class ActaSalidaAdmin(ExportMixin, admin.ModelAdmin):
	...
	form = ActaSalidaForm
	fieldsets = [(None,{'classes':('suit-tab','suit-tab-actasalida'),'fields':['custodio','destino','activos','descripcion','archivo']})]
...
class ActaEntradaAdmin(ExportMixin, admin.ModelAdmin):
	...
	fieldsets = [(None,{'classes':('suit-tab','suit-tab-actaentrada'),'fields':['custodio','origen','activos','descripcion','imagen']})]
...
class CompraAdmin(admin.ModelAdmin):
	...
	form = CompraForm
	fieldsets = [(None,{'classes':('suit-tab','suit-tab-actaentrada'),'fields':['custodio','descripcion','imagen']})]
	suit_form_tabs = (('actaentrada','1.Acta Entrada'), ('activo', '2.Activos'), ('entradanoserial','3.Activos no serial'))
#end class
