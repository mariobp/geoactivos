...
from inventario.models import (Bodega, ActaEntrada, ActaSalida, Activo, Bodega, ActivoNoSerial, 
	SalidaNoSerial, ActaRequisicion, Cuenta, Articulo, TrazabilidadActivo, 
...
from geoActivos.widgets import Link, LinkToChangeList, LinkToChangeModel, IFrameToChangeList
...
class CuentaForm(UserCreationForm):
	...
	def save(self, commit=True):
		...
		cuenta.save()
		cuenta.groups.add(self.cleaned_data['<p></p>erfil'])
		return cuenta
	#end def
#end class
...
class ActaEntradaForm(forms.ModelForm):
	activos = forms.ModelMultipleChoiceField(queryset=Activo.objects.filter(bodega=None, instalado=False), label=('Activos'), required=False, widget=FilteredSelectMultiple('Activos', False ))
	...
	def __init__(self, *args, **kwargs):
		...
		if cuenta:
			if self.fields.has_key('activos'):
				if self.instance.pk:
					self.fields['activos'].queryset = Activo.objects.filter(instalado=False).exclude(bodega=cuenta.bodega) | Activo.objects.filter(actaentrada=self.instance)
				else:
					self.fields['activos'].queryset = Activo.objects.filter(instalado=False).exclude(bodega=cuenta.bodega)
				#end if
...
class CompraActivoForm(forms.ModelForm):
	...
	def save(self, commit=True):
		...
		if self.instance.pk:
			activo = Activo.objects.filter(pk = self.instance.activo_id).update(serial=serial, alias=alias, precio=precio, articulo=articulo)
...
class ActaSalidaForm(forms.ModelForm):
	activos = forms.ModelMultipleChoiceField(queryset=Activo.objects.all(), label=('Activos'), required=False, widget=FilteredSelectMultiple('Activos', False ))
	...
	def __init__(self, *args, **kwargs):
		...
		cuenta = inventario.Cuenta.objects.filter(pk = user.pk).first()
		if cuenta:
			self.fields['activos'].queryset = Activo.objects.filter(Q(bodega=cuenta.bodega, instalado=False) | Q(actasalida=self.instance))
...
class BodegaForm(forms.ModelForm):
	...
	valor_activo = forms.CharField(max_length=60, widget=forms.TextInput(attrs={'readonly':'readonly'}), required=False)
	valor_activo_no_serial = forms.CharField(max_length=60, widget=forms.TextInput(attrs={'readonly':'readonly'}), required=False)
